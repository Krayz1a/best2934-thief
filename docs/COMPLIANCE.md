...
| 20 | A viewer application replays and verifies the log | Met | `ui/replay.py`, reached by **`p2pchase verify --log <file>`** (textual step-by-step replay plus commit-chain verification) and `p2pchase gui` (live board). This row named a `replay` subcommand, which has never existed — the capability was always there under another name, but a reader following the table would have got "invalid choice". Screenshots in README §9 |
